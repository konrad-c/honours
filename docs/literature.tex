...
Not only do the feature layer dimensions of each architecture differs, the information contained within these feature layers vary.
In order to make use of collections of these derived features from multiple classifiers, some research has used a technique known as stacking \citep{tsoumakas2002effective}.
The ensemble technique, in which multiple predictive models are trained and then used collectively to decide on the final prediction (often based on a majority vote of the models within it) resembles closely the stacking method.
Model stacking, often known as meta ensembling involves the use of multiple models with which to derive a larger set of feature information which is then used for the predictive process.
This has been used in various bodies of research, particularly in combination with existing image classifiers in order to derive a superset of features to further predict image emotion \citep{kim2018building} or even to synthesise photo-realistic images \citep{zhang2017stackgan}.
...
Techniques used in image emotion recognition have been used extensively in domains such as image sentiment analysis \citep{you2015robust}, and image-to-text synthesis \citep{vinyals2015show}.
However the subjectivity and human-dependent nature of emotion has naturally resulted in only small datasets of emotion-labelled images.
This is discussed in detail in the previous section, however the importance of domain agnostic and transferable predictive models is highlighted due to the particular case of image emotion and emotion as a target in the general case.
Through techniques such as transfer learning, pre-trained image object classifiers can and have been used in the domain of image emotion \citep{kim2018building, wangarttalk}.
