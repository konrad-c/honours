...
A model for either categorical classification or multi-dimensional regression, according to the given emotion representation, enables exploration of the desired feature-space.
...
Generating images according to the fitness function defined by the accuracy with which it is classified by the DNN, and storing individuals in the feature-space defined by the classes.
This process will result in a multi-dimensional feature-space in which each dimension represents a given emotion categorisation, divided into \textit{N} bins within which a generated image's classification accuracy falls.
The result is a collection of images spread throughout the emotion feature-space.

In the multi-dimensional regression variant, where the given image's VA/VAD values are computed, the feature-space is two or three dimensional, however there is no self-evident objective function for a generative framework to maximise.
Unlike the categorical classification accuracy used by \citet{nguyen2015deep,nguyen2015innovation}, a regression model bears no equivalent objective function.
As a result the associated VA/VAD feature-space will be explored using a conditional generative adversarial approach similar to that of \citet{tan2017artgan} and \citet{gauthier2014conditional}.
This technique involves the input of some condition, in some representation, to both generator and discriminator networks alongside their standard inputs.
The conditional GAN approach allows the generation of images conditioning on a point in the feature-space, with the associated objective function of discrimination accuracy.


\section{Expected Outcomes \& Contributions}

The outcomes of this project will include a generative system with which art can be synthesised according to a target emotional profile.
